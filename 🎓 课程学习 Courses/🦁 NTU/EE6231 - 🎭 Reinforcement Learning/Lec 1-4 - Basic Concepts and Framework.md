...
Yes, $p(r|s,a,s')$ is the complete form of reward function. However, $p(r|s,a)$ is also valid because the next state $s'$ depends on the current state $s$ and action $a$ by the transition probability $p(s'|s,a)$
