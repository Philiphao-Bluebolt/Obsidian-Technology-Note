...
主流编程语言都会提供封装好的序列容器或定义语法，如C语言的`var[]`（静态数组），`malloc`（可定义动态数组）；C++的标准库容器`<vector>`（动态数组）、`<list>`（链表）、`<array>`（静态数组）；Python的`list`（列表，即动态数组）、`tuple`（元组，即只读静态数组）；Java的`var[]`（静态数组）、`ArrayList`（动态数组）
