...
C++标准库  [algorithm](algorithm%20-%20数据结构算法.md) 封装了常用的 📦 [数据结构](📦%20数据结构及算法%20笔记导览.md) 操作算法，包括访问、插入、删除、查找、排序、计数等，支持所有标准容器。此外，所有容器类型都支持使用 👆️ [迭代器](迭代器%20Iterator.md) 遍历元素。
